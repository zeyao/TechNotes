...
- **先写数据库，再清除缓存**，会导致下一次读不到数据，可以在下一次**读缓存**的时候，如果读取缓存没找到，就读数据库，**在读的时候把缓存回写**。

- **先写缓存，再更新数据库**，可能导致缓存写成功，但是写数据库的时候响应失败，下次读取缓存会**脏读**， 这个方式是完全错误的
...
- **先清除缓存，再更新数据库**。这个逻辑是错误的，因为两个并发的**读和写操作**导致脏数据. 假如一个线程删除了缓存，更新数据库，此时正好有一个并发的**读操作**，读操作没有命中缓存后从数据库中取出老数据并且更新回缓存，这样缓存就不是应该更新的了。
